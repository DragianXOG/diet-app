...
def _set_rls(session: Session, uid: int) -> None:
    """
    For Postgres RLS that uses current_setting('app.user_id')::bigint,
    set the GUC on the SAME connection the Session will use.
    On SQLite / non-PG this safely no-ops.
    """
    try:
...
echo "✅ diet.py replaced and import OK."
