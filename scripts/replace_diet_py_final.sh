...
from app.models import User, Intake, Meal, MealItem  # NOTE: avoid GroceryItem mapping to bypass missing columns
...
def toggle_grocery_purchased(
    ...
    *,
    ...
    with _rls(session, user.id):
        ...
        current = bool(row[0])
        upd = text("UPDATE grocery_items SET purchased=:p WHERE id=:id AND user_id=:uid").bindparams(
...
def sync_groceries_from_meals(
    *,
    ...
    with _rls(session, user.id):
        if clear_existing:
            del_stmt = text("DELETE FROM grocery_items WHERE user_id=:uid AND purchased=:pfalse").bindparams(
...
echo "✅ diet.py replaced (final: bindparams + RLS + time-aware + raw SQL) and import OK."
