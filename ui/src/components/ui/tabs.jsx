...
export function TabsList({ children, className='' }) {
  return <div className={['flex flex-wrap gap-2 border rounded-2xl p-1   bg-[#4B0082]/10 tabs-strip  tabs-strip  backdrop-blur',className].join(' ')}>{children}</div>;
}
export function TabsTrigger({ value, children, className='' }) {
  const ctx = useContext(Ctx);
  const active = ctx.value === value;
  const base='px-3.5 py-2 rounded-xl text-sm transition border';
  const style = active ? 'bg-[#4B0082] text-white border-[#4B0082] shadow-soft' : 'text-foreground/70 border-transparent hover:  bg-[#4B0082]/10 tabs-strip  tabs-strip ';
